...
namespace Network.Clients
{
    public class MessageHandle
    {
        ...
        /// 解析数据或者叫做读取数据
        /// </summary>
        public void ReadMessage(int newDataAmount, Action<ActionCode, ReasonCode, string> processDataCallback)
        {
            ...
            while (true)
            {
                ...
                if ((startIndex - 4) >= count)
                {
                    ...
                    ReasonCode reasonCode = (ReasonCode)BitConverter.ToInt32(data, 8);
                    string s = Encoding.UTF8.GetString(data, 12, count - 8);
                    processDataCallback(actionCode, reasonCode, s);
